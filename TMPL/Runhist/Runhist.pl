...
perl Runhist.pl [-h|help|man] [-i|--input] <[file]> [-o|--output] <[file]> [-d|--default] [--parse] <[file]> [--save] <[file]> [--prompt] [--qsub]
...
The options are as follows:
 ...
 -o|--output 	Required. Specify the out <type of file> and/or full path.
...
if($default){
    $default_out = CreateDefaultOutput({class=>'Runhist'});
}

#Try to validate anything given from input and output flags
$valid_in = ValidatePath({dir=>$inputdir});
...
&Core::Runhist({input=>$valid_in, output=>$valid_out, qsub=>$qsub});
